...
import { Plus, Check, Edit2, Trash2, ChevronLeft, ChevronRight, Repeat } from 'lucide-react';
...
export function MobileDayCard({
  date, dayIndex, totalDays, tasks, stats,
  ...
  const handleAdd = () => {
    if (newTaskText.trim()) { onAddTask(newTaskText.trim()); setNewTaskText(''); setIsAdding(false); }
  };
  const handleSaveEdit = (taskId: string) => {
    if (editText.trim()) onUpdateTask(taskId, editText.trim());
    setEditingTaskId(null); setEditText('');
  };

  return (
    <div className="px-4 pb-24">
      {/* Header */}
      <div className={cn('flex items-center justify-between px-4 py-3 rounded-t-lg -mx-4 relative',
        allCompleted ? 'day-header-today' : (today ? 'day-header-today' : 'day-header'))}>
        <Button variant="ghost" size="icon" onClick={onPrevDay} disabled={dayIndex === 0}
          className="text-primary-foreground hover:bg-primary-foreground/10 h-10 w-10 disabled:opacity-30">
        ...
        <Button variant="ghost" size="icon" onClick={onNextDay} disabled={dayIndex === totalDays - 1}
          className="text-primary-foreground hover:bg-primary-foreground/10 h-10 w-10 disabled:opacity-30">
          <ChevronRight className="h-5 w-5" />
        </Button>
        {allCompleted && (
          <div className="absolute top-2 right-14 w-6 h-6 rounded-full bg-primary-foreground/25 flex items-center justify-center">
      ...
      <div className="flex flex-col items-center py-4 border-b border-border bg-card rounded-b-lg -mx-4 px-4">
        ...
        <div className="text-xs text-muted-foreground mt-1">{stats.completed} / {stats.total} done</div>
      </div>
          ...
          return (
            <div key={task.id} className="flex items-center gap-3 min-h-[44px] py-2 px-2 rounded bg-card">
              <button onClick={() => onToggleTask(task.id)}
                className={cn('w-7 h-7 rounded border-2 flex items-center justify-center shrink-0 transition-all',
              ...
              {editingTaskId === task.id ? (
                <Input value={editText} onChange={(e) => setEditText(e.target.value)}
                  ...
                  className="flex-1 h-10 text-sm" autoFocus />
              ) : (
              ...
              <div className="flex gap-0.5 shrink-0">
                <RecurrencePopover task={task} recurringDef={recurringDef} onSave={onSetRecurrence} compact={false} />
                <CopyDropdown task={task} date={date} days={days} onCopyToDay={onCopyToDay} onCopyToNextWeek={onCopyToNextWeek} compact={false} />
                <Button variant="ghost" size="icon" onClick={() => { setEditingTaskId(task.id); setEditText(task.text); }}
                  className="h-9 w-9 text-muted-foreground"><Edit2 className="h-3.5 w-3.5" /></Button>
                <Button variant="ghost" size="icon" onClick={() => onDeleteTask(task.id)}
                  className="h-9 w-9 text-destructive hover:bg-destructive/10"><Trash2 className="h-3.5 w-3.5" /></Button>
              </div>
            </div>
          );
        })}
        {tasks.length === 0 && <div className="text-center text-muted-foreground text-sm py-6 italic">No tasks yet</div>}
      </div>

      <div className="text-center text-xs text-muted-foreground mt-4">Day {dayIndex + 1} of {totalDays}</div>
      ...
      {isAdding && (
        <div className="fixed bottom-20 left-4 right-4 z-40 bg-card border border-border rounded-lg shadow-lg p-3 flex gap-2">
          <Input value={newTaskText} onChange={(e) => setNewTaskText(e.target.value)}
            placeholder="New task..." className="flex-1 h-11 text-sm"
            onKeyDown={(e) => e.key === 'Enter' && handleAdd()} autoFocus />
          <Button onClick={handleAdd} className="h-11 px-4"><Check className="h-4 w-4" /></Button>
        </div>
      )}
